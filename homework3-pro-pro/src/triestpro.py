...
class TriangleCounter:
    ...
    def __init__(self, file_path: str, max_memory: int):
        """
        ...
        self.edge_set: Set[FrozenSet[int]] = set()  # 样本里的edge e.g. self.edge_set = {frozenset({1, 2}), frozenset({2, 3}), frozenset({3, 4})}
        self.edge_count: int = 0  # num of total read edges
        self.vertex_triangle_map: DefaultDict[int, int] = defaultdict(int)  # num of triangles every vertex takes part in
    ...
    def should_store_edge2(self, current_count: int) -> bool:
        """
        Improved Function
        """
        if current_count <= self.max_memory:
    ...
    def execute(self) -> float:
        with open(self.file_path, 'r') as data_file:
            ...
            for line in data_file:
                ...
                self.edge_count += 1
                self.update_triangle_counters(lambda x, y: x + y, edge)
                if self.should_store_edge2(self.edge_count):
                    self.edge_set.add(edge)
